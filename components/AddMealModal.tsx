...
interface AddMealModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { foodName: string; nutrition: NutritionInfo }) => void;
}
...
export const AddMealModal: React.FC<AddMealModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!foodName.trim()) {
        // Basic validation: ensure food name is not empty
        return;
    }
    onSave({ foodName, nutrition });
  };
    ...
    >
      ...
      <div className="relative bg-slate-900/70 backdrop-blur-lg border border-cyan-300/20 rounded-2xl shadow-xl w-full max-w-md m-4 animate-fade-in">
        <div className="p-6 border-b border-cyan-300/20">
          <h2 id="add-meal-title" className="text-xl font-bold text-slate-100">Add Meal Manually</h2>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full px-3 py-2 border border-slate-600 rounded-md shadow-sm focus:ring-cyan-400 focus:border-cyan-400 bg-slate-800 text-slate-200 placeholder:text-slate-500"
                required
              />
            </div>
            <div className="grid grid-cols-2 gap-4">
              {Object.keys(nutrition).map((key) => (
                 <div key={key}>
                    <label htmlFor={`add-${key}`} className="block text-sm font-medium text-slate-400 mb-1 capitalize">{key} {key !== 'calories' && '(g)'}</label>
                    <input 
                      type="number" 
                      id={`add-${key}`}
                      name={key}
          ...
          <div className="bg-slate-900/50 p-4 flex justify-end space-x-3 rounded-b-2xl border-t border-cyan-300/20">
            ...
            <button 
              type="submit"
              className="px-4 py-2 bg-gradient-to-r from-purple-500 to-cyan-400 text-white font-semibold rounded-full hover:shadow-md hover:shadow-cyan-500/20 transition-all active:scale-95 disabled:from-slate-600 disabled:to-slate-700 disabled:shadow-none disabled:cursor-not-allowed"
